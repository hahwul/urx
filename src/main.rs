...
use cli::{Args, read_domains_from_stdin};
use providers::{CommonCrawlProvider, Provider, WaybackMachineProvider, OTXProvider};
use output::create_outputter;
use testers::{StatusChecker, LinkExtractor, Tester};
...
async fn main() -> Result<()> {
    ...
    if args.providers.iter().any(|p| p == "wayback") {
        if args.verbose {
            ...
            if args.proxy.is_some() {
                println!("Using proxy for Wayback Machine: {}", args.proxy.as_ref().unwrap());
            }
        }
        ...
        if args.verbose {
            println!("Timeout set to {} seconds for Wayback Machine", args.timeout);
            println!("Retries set to {} for Wayback Machine", args.retries);
            println!("Parallel requests set to {} for Wayback Machine", args.parallel);
            if let Some(rate) = args.rate_limit {
                println!("Rate limit set to {} requests/second for Wayback Machine", rate);
            }
        }
    ...
    if args.providers.iter().any(|p| p == "cc") {
        if args.verbose {
            ...
            if args.proxy.is_some() {
                println!("Using proxy for Common Crawl: {}", args.proxy.as_ref().unwrap());
            }
        }
        ...
        if args.verbose {
            ...
            println!("Parallel requests set to {} for Common Crawl", args.parallel);
            if let Some(rate) = args.rate_limit {
                println!("Rate limit set to {} requests/second for Common Crawl", rate);
            }
        }
    ...
    for (idx, domain) in domains.into_iter().enumerate() {
        ...
        if args.verbose {
            println!("Processing domain [{}/{}]: {}", idx + 1, total_domains, domain);
        }
    ...
    let filter_bar = if !args.extensions.is_empty() || !args.patterns.is_empty() || 
                       !args.exclude_extensions.is_empty() || !args.exclude_patterns.is_empty() ||
                       args.min_length.is_some() || args.max_length.is_some() {
    ...
    url_filter.with_extensions(args.extensions.clone())
    ...
    let transform_bar = if args.merge_endpoint || args.show_only_host || args.show_only_path || args.show_only_param {
    ...
    if args.check_status || args.extract_links {
        ...
        for (chunk_idx, url_chunk) in url_chunks.into_iter().enumerate() {
            ...
            test_bar.set_message(format!("Processing chunk {}/{}", chunk_idx + 1, chunk_count));
        }
